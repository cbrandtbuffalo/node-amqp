...
Queue.prototype._onMethod = function (channel, method, args) {
  ...
  switch (method) {
    ...
    case methods.basicGetOk:
      debug('basicGetOk', util.inspect(args, null));
      var m = this.currentMessage = new Message(this, args);
      var buffer = '';
      this.currentMessage.addListener('data', function(d) {
         buffer += d.toString();
      });
      this.currentMessage.addListener('end', function() {
        self.getMessageListener(buffer);
      });
      break;

    case methods.basicGetEmpty:
      debug('basicGetEmpty', util.inspect(args, null));
      this.currentMessage = new Message(this, args);
      self.getMessageListener() 
      break;
...
Queue.prototype.get = function(options, messageListener) { // ➔ get-ok | get-empty
    var self = this;
    var buffer = '';
    options = options || {}
    if (typeof(options) == "function") {
    ...
    this.getMessageListener = messageListener
    return this._taskPush(methods.get, function () {
      self.connection._sendMethod(self.channel, methods.basicGet, {
        reserved1: 0,
        queue: self.name,
        noAck: !!options['noack']
      });
